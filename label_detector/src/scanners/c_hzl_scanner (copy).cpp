...
{	
 ...
 //    }
	...
	// vector<HzlLabel> labels;
	// if (_match_method == 0)    // Selected method: matchTemplate
	// 	labels = getHazardousLabels(_image_to_scan, label_contours);
	// else if (_match_method == 1)    // Selected method: customMatching
	// 	labels = getHazardousLabels(_image_to_scan, caminput_gray, label_contours);
	...
	if ( contours.size() == approximated_contours.size() ) {   // Just in case..
		...
		for ( size_t k = 0; k < approximated_contours.size(); k++) {
			...
			bool is_possible_label = approximated_contours[k].size() == 4 &&    // It is quadrilateral
									 ...
									 contourArea(approximated_contours[hierarchy[k][2]]) > (contourArea(approximated_contours[k]) * 0.6) &&
...
{   
    ...
    if ( whatmethod  == cv::TM_SQDIFF || whatmethod == cv::TM_SQDIFF_NORMED )    return 1 - mean[0]; 
    else    return mean[0];
}
