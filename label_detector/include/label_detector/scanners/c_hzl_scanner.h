...
private:
	...
	std::vector<HzLabelTemplate> _labels_templates;
	std::vector<Point2D> _perspective_square;

	bool loadLabels();
	std::vector< std::vector<Point2D> > findLabelContours(const cv::Mat& canny_image);
	std::vector<HzlLabel> getHazardousLabels(shared_ptr<cv::Mat> image, const cv::Mat& gray_image, const std::vector<std::vector<Point2D> >& contours);
	...
	template<typename T>
	std::vector<Point2D> convertToPoint2D(const std::vector<T> input_vector);
	template<typename T>
	std::vector<ContourPoint> convertToContourPoint(const std::vector<T> input_vector);
	template<typename T>
	void normalizeVector(std::vector<T>& vector);
	template<typename Ta, typename Tb>
	Ta findMiddlePoint(const Ta& pointA, const Tb& pointB);

	void drawMatches(cv::Mat &inputimage, std::vector<HzlLabel> &labels);
